...
static PixelFormat GetPixelFormatForDrmFormat(uint32_t format)
{
	switch (format)
	{
		case DRM_FORMAT_UYVY: return PixelFormat::UYVY;
		case DRM_FORMAT_YUYV: return PixelFormat::YUYV;
		case DRM_FORMAT_BGR565: return PixelFormat::BGR16;
		case DRM_FORMAT_BGR888: return PixelFormat::BGR24;
			...
			for(auto & framebuffer : framebuffers)
			{
				...
				else if (_pixelFormat == PixelFormat::NV12 && modifier >> 56ULL == DRM_FORMAT_MOD_VENDOR_BROADCOM)
				{
					switch (fourcc_mod_broadcom_mod(modifier))
					{
						...
						{
							Debug(_log, "DRM_FORMAT_MOD_BROADCOM_VC4_T_TILED");
							break;
						}
